...
class ClassicInterface(QMainWindow):
    ...
    def initUI(self):
        ...
        self.add_label_button.clicked.connect(self.add_new_label_type)
    ...
    def update_widget_states(self):
        ...
        if has_video or has_keypoints:
            ...
            self.slider.setMaximum(0); self.frame_index = 0
    ...
    def load_keypoints(self):
        ...
        if keypoints_file:
            ...
            if keypoints_data is None:
                self.display_error_message("Keypoint Load Error", f"Failed load: {keypoints_file}");
                return
            if keypoints_data.ndim != 3 or keypoints_data.shape[-1] != 3:
                self.display_error_message("Keypoint Data Error",
                                           f"Invalid shape: {keypoints_data.shape}. Expected (frames, points, 3).");
                return
            ...
            self.update_widget_states()
            self.display_video_frame()  # Ensure video frame is updated if video is already loaded
    ...
    def load_label_names(self):
        ...
        if label_file:
            try:
                ...
                if not label_names: self.show_status_message("Warning: Label names file empty.", 5000); return
                existing_names = {ld.name for ld in self.label_data_list};
                new_labels_added = 0
                for name in label_names:
                    if name not in existing_names: self.add_label_type(name=name); existing_names.add(
                        name); new_labels_added += 1
                self.show_status_message(f"Loaded {len(label_names)} names. Added {new_labels_added} new types.", 5000);
                self.update_widget_states()
            except FileNotFoundError:
                self.display_error_message("Load Error", f"File not found: {label_file}")
            except Exception as e:
                self.display_error_message("Load Error", f"Failed load names: {e}")
    ...
    def update_frame_display(self):
        ...
        if self.keypoints is not None and self.openGLWidget.isEnabled(): self.openGLWidget.set_frame_index(
            self.frame_index)
    ...
    def add_new_label_type(self):
        self.add_label_type()

    def add_label_type(self, name="", is_numeric=False, values=None):
        if values is None: values = []
        label_data = LabelData(name=name, is_numeric=is_numeric);
        label_data.values = values
        self.label_data_list.append(label_data);
        label_widgets = self._create_label_ui(label_data);
        self.ui_widgets[label_data] = label_widgets;
        self.update_widget_states()

    def _create_label_ui(self, label_data):
        label_type_widget = QWidget(self);
        label_type_widget.setStyleSheet("QWidget { border: 1px solid #e0e0e0; margin-bottom: 5px; padding: 3px; }")
        label_type_layout = QVBoxLayout(label_type_widget);
        label_type_layout.setContentsMargins(2, 2, 2, 2);
        label_type_layout.setSpacing(3)
        header_layout = QHBoxLayout();
        label_name_input = QLineEdit(label_data.name, self);
        label_name_input.setPlaceholderText("Enter Label Name")
        label_name_input.textChanged.connect(lambda text, ld=label_data: setattr(ld, 'name', text.strip()))
        numeric_checkbox = QCheckBox("Numeric (Zero Padding)", self);
        numeric_checkbox.setChecked(label_data.is_numeric)
        numeric_checkbox.stateChanged.connect(
            lambda state, ld=label_data: setattr(ld, 'is_numeric', state == Qt.CheckState.Checked.value))
        type_collapse_button = QToolButton(self);
        type_collapse_button.setArrowType(Qt.ArrowType.DownArrow);
        type_collapse_button.setCheckable(True);
        type_collapse_button.setChecked(True);
        type_collapse_button.setStyleSheet("QToolButton { border: none; }")
        header_layout.addWidget(QLabel("Name:"));
        header_layout.addWidget(label_name_input, stretch=1);
        header_layout.addWidget(numeric_checkbox);
        header_layout.addWidget(type_collapse_button)
        label_type_layout.addLayout(header_layout)
        content_area = QWidget(self);
        content_layout = QVBoxLayout(content_area);
        content_layout.setContentsMargins(5, 0, 0, 0);
        content_layout.setSpacing(2)
        label_type_layout.addWidget(content_area);
        add_value_button = QPushButton("Add Value/Category", self);
        content_layout.addWidget(add_value_button)
        values_container_layout = QVBoxLayout();
        values_container_layout.setSpacing(2);
        content_layout.addLayout(values_container_layout)
        type_collapse_button.toggled.connect(
            lambda checked, ca=content_area, btn=type_collapse_button: self._toggle_visibility(checked, ca, btn))
        add_value_button.clicked.connect(
            lambda _, ld=label_data, vcl=values_container_layout: self._add_value_category(ld, vcl))
        for value_str, intervals_list in label_data.values: self._create_value_ui(label_data, value_str, intervals_list,
                                                                                  values_container_layout)
        self.labels_layout.addWidget(label_type_widget)
        return {"main_widget": label_type_widget, "name_input": label_name_input, "numeric_checkbox": numeric_checkbox,
                "collapse_button": type_collapse_button, "content_area": content_area,
                "values_container": values_container_layout, "add_value_button": add_value_button}

    def _add_value_category(self, label_data, values_container_layout):
        new_value = "";
        new_intervals = []
        label_data.values.append((new_value, new_intervals))
        self._create_value_ui(label_data, new_value, new_intervals, values_container_layout)
    ...
    def _create_value_ui(self, label_data, value_str, intervals_list_ref, values_container_layout):
        ...
        if value_data_index == -1: return
        value_widget = QWidget(self);
        value_widget.setStyleSheet("QWidget { border: 1px solid #f5f5f5; margin-bottom: 3px; padding: 2px; }")
        value_layout = QVBoxLayout(value_widget);
        value_layout.setContentsMargins(2, 2, 2, 2);
        value_layout.setSpacing(2)
        value_header_layout = QHBoxLayout();
        value_input = QLineEdit(value_str, self);
        value_input.setPlaceholderText("Enter Value/Category")
        value_input.textChanged.connect(
            lambda text, ld=label_data, idx=value_data_index: self._update_value_string(ld, idx, text))
        ...
        value_collapse_button.setChecked(True);
        value_collapse_button.setStyleSheet("QToolButton { border: none; }")
        delete_value_button = QPushButton("X", self);
        delete_value_button.setFixedSize(QSize(20, 20));
        delete_value_button.setStyleSheet("QPushButton { color: red; font-weight: bold; border: none; }");
        delete_value_button.setToolTip("Delete this value/category")
        value_header_layout.addWidget(QLabel("Value:"));
        value_header_layout.addWidget(value_input, stretch=1);
        value_header_layout.addWidget(value_collapse_button);
        value_header_layout.addWidget(delete_value_button);
        value_layout.addLayout(value_header_layout)
        intervals_area = QWidget(self);
        intervals_area_layout = QVBoxLayout(intervals_area);
        intervals_area_layout.setContentsMargins(5, 0, 0, 0);
        intervals_area_layout.setSpacing(2);
        value_layout.addWidget(intervals_area)
        add_interval_button = QPushButton("Add Interval", self);
        intervals_area_layout.addWidget(add_interval_button)
        intervals_container_layout_for_this_value = QVBoxLayout();
        intervals_area_layout.addLayout(intervals_container_layout_for_this_value)
        value_collapse_button.toggled.connect(
            lambda checked, ia=intervals_area, btn=value_collapse_button: self._toggle_visibility(checked, ia, btn))
        add_interval_button.clicked.connect(lambda _, ld=label_data, il_ref=intervals_list_ref,
                                                   icl=intervals_container_layout_for_this_value: self._add_interval(ld,
                                                                                                                     il_ref,
                                                                                                                     icl))
        delete_value_button.clicked.connect(
            lambda _, ld=label_data, idx=value_data_index, vw=value_widget: self._delete_value_category(ld, idx, vw))
        for interval_pair_ref in intervals_list_ref:
            if isinstance(interval_pair_ref, list) and len(interval_pair_ref) == 2: self._create_interval_ui(label_data,
                                                                                                             intervals_list_ref,
                                                                                                             interval_pair_ref,
                                                                                                             intervals_container_layout_for_this_value)
        values_container_layout.addWidget(value_widget)

    def _update_value_string(self, label_data, index, new_text):
        if 0 <= index < len(label_data.values): label_data.values[index] = (
        new_text.strip(), label_data.values[index][1])

    def _delete_value_category(self, label_data, index, value_widget):
        if 0 <= index < len(label_data.values): label_data.values.pop(
            index); value_widget.deleteLater(); self.show_status_message("Value category removed.", 3000)

    def _add_interval(self, label_data, intervals_list_ref, intervals_container_layout):
        new_interval_pair = [0, 0];
        intervals_list_ref.append(new_interval_pair)
        self._create_interval_ui(label_data, intervals_list_ref, new_interval_pair, intervals_container_layout)

    def _create_interval_ui(self, label_data, intervals_list_ref, interval_pair_ref, intervals_container_layout):
        start_frame = interval_pair_ref[0] if len(interval_pair_ref) > 0 else 0;
        end_frame = interval_pair_ref[1] if len(interval_pair_ref) > 1 else 0
        interval_widget = QWidget(self);
        interval_layout = QHBoxLayout(interval_widget);
        interval_layout.setContentsMargins(0, 0, 0, 0)
        start_frame_input = QLineEdit(str(start_frame), self);
        end_frame_input = QLineEdit(str(end_frame), self)
        start_frame_input.textChanged.connect(
            lambda text, ref=interval_pair_ref: self._update_interval_frame(ref, 0, text))
        end_frame_input.textChanged.connect(
            lambda text, ref=interval_pair_ref: self._update_interval_frame(ref, 1, text))
        delete_interval_button = QPushButton("x", self);
        delete_interval_button.setFixedSize(QSize(15, 15));
        delete_interval_button.setStyleSheet("QPushButton { color: red; border: none; }");
        delete_interval_button.setToolTip("Delete interval")
        interval_layout.addWidget(QLabel("Start:"));
        interval_layout.addWidget(start_frame_input);
        interval_layout.addWidget(QLabel("End:"));
        interval_layout.addWidget(end_frame_input);
        interval_layout.addWidget(delete_interval_button)
        delete_interval_button.clicked.connect(
            lambda _, il_ref=intervals_list_ref, ip_ref=interval_pair_ref, iw=interval_widget: self._delete_interval(
                il_ref, ip_ref, iw))
        intervals_container_layout.addWidget(interval_widget)

    def _update_interval_frame(self, interval_pair_ref, index_in_pair, text):
        try:
            if isinstance(interval_pair_ref, list) and 0 <= index_in_pair < 2: interval_pair_ref[index_in_pair] = int(
                text.strip())
        except ValueError:
            if isinstance(interval_pair_ref, list) and 0 <= index_in_pair < 2: interval_pair_ref[index_in_pair] = 0

    def _delete_interval(self, intervals_list_ref, interval_pair_ref, interval_widget):
        try:
            intervals_list_ref.remove(interval_pair_ref); interval_widget.deleteLater(); self.show_status_message(
                "Interval removed.", 2000)
        except ValueError:
            pass

    def _toggle_visibility(self, checked, widget_to_toggle, button):
        widget_to_toggle.setVisible(checked); button.setArrowType(
    ...
    def save_csv(self):
        ...
        if not self.label_data_list or not has_named_labels:
            self.display_error_message("Save Error", "No labels defined or named.")
            return
        base_name = os.path.splitext(self.keypoints_path)[0] if self.keypoints_path else "classic_labeled";
        suggested_path = f"{base_name}_labels.csv"
        ...
        self.show_status_message(f"Saving to {save_file}...");
        ...
        try:
            ...
            for ld in self.label_data_list:
                if ld.name:
                    ...
                    label_names.append(ld.name);
                    valid_label_data.append(ld)
                else:
                    warnings.warn("Skipping label type with empty name during save.", UserWarning); warnings_list.add(
                        "empty_label")
            if not valid_label_data: raise ValueError("No valid labels with names found.")
            ...
            for frame_idx in range(total_frames):
                ...
                for kp_idx, point in enumerate(self.keypoints[frame_idx]):
                    if len(point) >= 3:
                        frame_data[f"kp{kp_idx}_x"] = point[0]; frame_data[f"kp{kp_idx}_y"] = point[1]; frame_data[
                            f"kp{kp_idx}_z"] = point[2]
                    else:
                        frame_data[f"kp{kp_idx}_x"] = np.nan; frame_data[f"kp{kp_idx}_y"] = np.nan; frame_data[
                            f"kp{kp_idx}_z"] = np.nan; warnings_list.add(f"kp_dim_{kp_idx}")
                for label_data in valid_label_data:
                    label_name = label_data.name;
                    value_for_frame = None
                    for value_str, intervals_list in label_data.values:
                        for interval_pair in intervals_list:
                            try:
                                ...
                                start_f = int(interval_pair[0]);
                                end_f = int(interval_pair[1])
                                if start_f <= frame_idx <= end_f: value_for_frame = value_str; break
                            except (ValueError, TypeError, IndexError):
                                warnings_list.add(f"interval_{label_name}"); continue
                        if value_for_frame is not None: break
                    if value_for_frame is not None:
                        if label_data.is_numeric:
                            try:
                                frame_data[label_name] = float(value_for_frame)
                            except (ValueError, TypeError):
                                frame_data[label_name] = 0.0; warnings_list.add(f"numeric_{label_name}")
                        else:
                            frame_data[label_name] = value_for_frame
                    else:
                        frame_data[label_name] = 0.0 if label_data.is_numeric else ""
                data_to_save.append(frame_data)
                if (frame_idx + 1) % 100 == 0: self.show_status_message(f"Saving... {frame_idx + 1}/{total_frames}",
                                                                        0); QApplication.processEvents()
            df = pd.DataFrame(data_to_save, columns=headers);
            df.to_csv(save_file, index=False, na_rep='NaN')
            if warnings_list: print(
                f"\n--- Save Warnings ---\nUnique issues encountered: {', '.join(sorted(list(warnings_list)))}\n-------------------\n")
            self.show_status_message(f"Saved to {save_file}", 5000)
        except ValueError as ve:
            self.display_error_message("Save Error", f"{ve}"); self.show_status_message("Save failed.", 5000)
        except Exception as e:
